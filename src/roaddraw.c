...
/* Swap a 16-bit pixel from big-endian pack data to native endian */
#ifdef __BIG_ENDIAN__
  #define PIXEL16(x) (x)
#else
  #define PIXEL16(x) SWAP16(x)
#endif
...
void DrawRoadZoomed(float xDrawStart,float yDrawStart,float zoom)
{
	...
	if(drawPos!=gBaseAddr+640*(gPrefs.lineSkip?240:480)+(gPrefs.lineSkip?240:480)*rowBytesSkip)DoError(-50);
}
...
Ptr DrawLineZoomed16(UInt16 *drawPos,int xDrawStart,int x1,int x2,int y,int xDrift,int yDrift,UInt16 *data,float zoom)
{
	...
	while(x2)
	{
		*(drawPos++)=PIXEL16(data[((u>>8)&0x007f)]);
		u+=dudx;
		x2--;
	}
	return drawPos;
...
void DrawRoadZoomed16(float xDrawStart,float yDrawStart,float zoom)
{
	...
	if(drawPos!=gBaseAddr+640*(gPrefs.lineSkip?240:480)*2+(gPrefs.lineSkip?240:480)*rowBytesSkip)DoError(-50);
}
