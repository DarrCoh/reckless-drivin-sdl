...
static Ptr DrawLineZoomed(Ptr drawPos,int xDrawStart,int x1,int x2,int y,int xDrift,int yDrift,Ptr data,float zoom)
{
	UInt32 u,v;
	UInt32 dudx;
	if(x1<0)x1=0;
...
static Ptr DrawBorderZoomed(Ptr drawPos,int xDrawStart,int x1,int x2,int y,Ptr data,float zoom)
{
	UInt32 dudx;
	UInt32 v=(-y&0x007f)<<4;
	UInt32 u=0;
...
static void DrawRoadZoomed(float xDrawStart,float yDrawStart,float zoom)
{
...
static void DrawMarksZoomed(float xDrawStart,float yDrawStart,float zoom)
{
	float invZoom=1/zoom;
...
static void DrawTracksZoomed(float xDrawStart,float yDrawStart,float zoom)
{
	float invZoom=1/zoom;
	int i;
	int yClip=gYSize-(gFinishDelay?0:kInvLines);
...
static void DrawSpriteLayerZoomed(float xDrawStart,float yDrawStart,float zoom,float tide,int layer)
{
...
static void DrawSpriteLayerBlurZoomed(float xDrawStart,float yDrawStart,float zoom,float tide,int layer)
{
	tObject *theObj=gFirstVisObj;
	float invZoom=1/zoom;
	while(theObj!=gLastVisObj)
...
static void DrawSpritesZoomed(float xDrawStart,float yDrawStart,float zoom)
{
	float invZoom=1/zoom;
	int layer;
